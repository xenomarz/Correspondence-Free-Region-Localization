...
private:
	...
	void InitializeTriplets(std::vector<Eigen::Triplet<double>>& triplets) override
	{
		...
		for (auto column = 0; column < this->objective_variables_count_; column++)
		{
	...
	void InitializeMappings(HessianEntryToTripletIndexMap& hessian_entry_to_triplet_index_map, SparseVariableIndexToDenseVariableIndexMap& sparse_variable_index_to_dense_variable_index_map) override
	{
		auto triplet_index = 0;
		for (auto column = 0; column < this->objective_variables_count_; column++)
		{
			for (auto row = 0; row <= column; row++)
			{
				hessian_entry_to_triplet_index_map[{ row, column }] = triplet_index;
				triplet_index++;
			}
		}
	}
	...
	std::pair<int64_t, int64_t> edge1_indices_;
	std::pair<int64_t, int64_t> edge2_indices_;
	...
	std::unordered_map<std::pair<PartialCoordinateType, Partial>, double, RDS::OrderedPairHash, RDS::OrderedPairEquals> partial_to_second_derivative_value_map_;
};
