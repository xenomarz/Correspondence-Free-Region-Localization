...
public:
	 ...
	 */
	EdgePairLengthObjective(const std::shared_ptr<MeshDataProvider>& mesh_data_provider, const std::shared_ptr<EdgePairDataProvider>& edge_pair_data_provider, const bool enforce_psd = false) :
		EdgePairObjective(mesh_data_provider, edge_pair_data_provider, "Edge Pair Length Objective", enforce_psd)
	{
		this->Initialize();
	}
